...
async function checkUsername(username) {
  ...
  const sites = [
    ...
    { name: 'Threads', url: `https://www.threads.net/@${u}`,
      check: r => r.status === 200 && !String(r.data).includes('Sorry, this page'), uncertain: true },
    { name: 'Tumblr', url: `https://${u}.tumblr.com`,
      check: r => r.status === 200 && !String(r.data).includes('There\'s nothing here') },
    ...
    { name: 'Linktree', url: `https://linktr.ee/${u}`,
      check: r => r.status === 200 && !String(r.data).includes('Page Not Found') },
    { name: 'Wattpad', url: `https://www.wattpad.com/user/${u}`,
      check: r => r.status === 200 },
    { name: 'PSN Profiles', url: `https://psnprofiles.com/${u}`,
      check: r => r.status === 200 },
    ...
    { name: 'Replit', url: `https://replit.com/@${u}`,
      check: r => r.status === 200 },
    { name: 'Codewars', url: `https://www.codewars.com/users/${u}`,
      check: r => r.status === 200 },
    { name: 'Hacker News', url: `https://news.ycombinator.com/user?id=${u}`,
      check: r => r.status === 200 && !String(r.data).includes('No such user') },
    { name: 'Kaggle', url: `https://www.kaggle.com/${u}`,
      check: r => r.status === 200 },
